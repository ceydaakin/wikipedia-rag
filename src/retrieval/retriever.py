...
from config import COMPARISON_TOP_K_PER_ENTITY, DEFAULT_TOP_K
...
def retrieve(query: str, top_k: int = DEFAULT_TOP_K) -> RetrievalResult:
    ...
    - If 2+ specific entities are mentioned (comparison), retrieve
      COMPARISON_TOP_K_PER_ENTITY chunks per named entity, with no other filter.
    ...
    if analysis.is_comparison and len(analysis.matched_entities) >= 2:
        ...
        return RetrievalResult(analysis=analysis, chunks=bag)
    ...
    if len(analysis.matched_entities) == 1:
        ...
        return RetrievalResult(analysis=analysis, chunks=chunks)
